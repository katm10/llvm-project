...
static llvm::cl::opt<bool> isUserVersion("user-version", llvm::cl::desc("Use user version of the function"), llvm::cl::init(false));
...
public:
	...
	bool VisitCallExpr(CallExpr *call) {
		...
		SourceLocation SL = call->getBeginLoc();
		...
		if (call->getDirectCallee() == nullptr) {
			llvm::errs() << "Indirect call in " << current_function << "\n";	
			Expr * target = call->getCallee();

			SourceLocation start = target->getBeginLoc();
			SourceLocation end = target->getEndLoc();

			SourceLocation e(Lexer::getLocForEndOfToken(end, 0, ConditionRewriter.getSourceMgr(), 
				ConditionRewriter.getLangOpts()));
			

			std::stringstream SSBefore;

			QualType ptr_type = target->getType();
			std::string ptr_type_str = ptr_type.getAsString();
			

			SSBefore << "((" << ptr_type_str << ")__translate_function(";
			ConditionRewriter.InsertText(start, SSBefore.str(), true, true);	
			std::stringstream SSAfter;
			SSAfter << "))";
			ConditionRewriter.InsertText(e, SSAfter.str(), true, true);
	...
	bool VisitFunctionDecl(FunctionDecl *f) {
      ...
      llvm::errs() << "Found function " << current_function << "\n";
			
			std::stringstream SSAfter;
      const char* function_suffix = (isUserVersion) ? "_slow__addr" : "__addr";
			SSAfter << "void* __attribute__((weak)) " << current_function << function_suffix << "(void) { return (void*)&"				<< current_function << ";}\n";
